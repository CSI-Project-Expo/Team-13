                            ...
                            <>
                                ...
                                <div className="stats-grid">
                                    <StatCard
                                        ...
                                        value={`₹${Number(dashboard?.total_balances?.balance ?? 0).toFixed(2)}`}
                                        accent
                                    />
                                    <StatCard
                                        label="Total Escrow"
                                        value={`₹${Number(dashboard?.total_balances?.escrow ?? 0).toFixed(2)}`}
                                    />
                                    <StatCard
                                ...
                                {dashboard?.recent_jobs?.length > 0 ? (
                                    <div className="admin-table-wrap">
                                        <table className="admin-table">
                                            <thead>
                                                <tr>
                                                    <th>Title</th><th>Status</th><th>User</th><th>Genie</th><th>Date</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {dashboard.recent_jobs.map((j) => (
                                                    <tr key={j.id} className="admin-table__row">
                                                        ...
                                                        <td className="admin-table__cell"><StatusBadge status={j.status} /></td>
                                                        <td className="admin-table__cell">{j.user || '—'}</td>
                                                        <td className="admin-table__cell">{j.genie || '—'}</td>
                                                        <td className="admin-table__cell">
                                ...
                                {users.length === 0 ? (
                                    ...
                                    <div className="admin-table-wrap">
                                        <table className="admin-table">
                                            <thead>
                                                <tr>
                                                    <th>Name</th><th>Role</th><th>Joined</th><th>Change Role</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {users.map((u) => (
                                ...
                                {jobs.length === 0 ? (
                                    ...
                                    <div className="admin-table-wrap">
                                        <table className="admin-table">
                                            <thead>
                                                <tr>
                                                    <th>Title</th><th>Status</th><th>Price</th><th>Location</th><th>Posted</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {jobs.map((j) => (
                                                    <tr key={j.id} className="admin-table__row">
                                                        ...
                                                        <td className="admin-table__cell">{j.location || '—'}</td>
                                                        <td className="admin-table__cell">
                                                            {new Date(j.created_at).toLocaleDateString('en-IN')}
                                                        </td>
                                                    </tr>
